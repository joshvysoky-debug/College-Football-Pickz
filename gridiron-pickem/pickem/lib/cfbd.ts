...
/**
 * Schools where CFBD's `school` field and ESPN's `team.location` field are
 * genuinely different words/abbreviations for the same program, so no
 * amount of punctuation/case normalization makes them line up. Keyed
 * either direction (order doesn't matter) as normalized strings.
 *
 * This is a best-effort starting list, not exhaustive — the real fix is
 * that fetchLiveScoreboard now reports which games it couldn't match (see
 * `unmatched` below and app/api/sync/live/route.ts) instead of just
 * silently leaving that game's card frozen. Add to this list as real
 * mismatches surface there, rather than guessing more up front.
 */
...
/** Every normalized spelling a school might plausibly appear under. */
function aliasCandidates(normalized: string): string[] {
  const candidates = [normalized];
  for (const [a, b] of Object.entries(KNOWN_NAME_ALIASES)) {
    if (normalized === a) candidates.push(b);
    else if (normalized === b) candidates.push(a);
  }
  return candidates;
}
